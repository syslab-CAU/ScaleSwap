...
DECLARE_PER_CPU(long, p_nr_swap_pages);
extern unsigned long long total_swap_pages;
extern atomic_t nr_rotate_swap;
extern bool has_usable_swap(void);

extern long (*get_nr_swap_pages_module)(long over_val, bool is_bigger);
static inline unsigned long long get_nr_swap_pages(unsigned long long over_val, bool is_bigger)
{
	if (get_nr_swap_pages_module)
		return get_nr_swap_pages_module(over_val, is_bigger);

	unsigned long long sum = 0;
	int cpu;
	for_each_online_cpu(cpu) {
		sum += per_cpu(p_nr_swap_pages, cpu);
		if (over_val != -1) {
			if (is_bigger && over_val <= sum) 
				return sum;
			else if (!is_bigger && over_val > sum) 
				return sum;
		}
	}
