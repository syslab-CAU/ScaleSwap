...
EXPORT_SYMBOL_GPL(workingset_age_nonresident); // profiling
...
//	lruvec = mem_cgroup_lruvec(target_memcg, pgdat, folio_to_cpu(folio));
	lruvec = my_mem_cgroup_lruvec(target_memcg, pgdat, folio_to_cpu(folio));
...
EXPORT_SYMBOL_GPL(workingset_eviction); // profiling
...
bool workingset_test_recent(void *shadow, bool file, bool *workingset)
{
	...
	eviction_lruvec = mem_cgroup_lruvec(eviction_memcg, pgdat, -1);
...
EXPORT_SYMBOL(workingset_test_recent);
...
EXPORT_SYMBOL_GPL(workingset_refault); // profiling
...
// EXPORT_SYMBOL_GPL(swap_task_handler_work);
...
EXPORT_SYMBOL_GPL(workingset_update_node); // profiling
					...
					struct shrink_control *sc)
{
	...
	if (sc->memcg) {
		...
		lruvec = mem_cgroup_lruvec(sc->memcg, NODE_DATA(sc->nid), -1);
