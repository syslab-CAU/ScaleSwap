...
EXPORT_SYMBOL_GPL(mm_trace_rss_stat); // profiling
...
int (*__pte_alloc_module)(struct mm_struct *mm, pmd_t *pmd) = NULL;
EXPORT_SYMBOL(__pte_alloc_module);
int __pte_alloc(struct mm_struct *mm, pmd_t *pmd)
{
	if (__pte_alloc_module) 
		return (*__pte_alloc_module)(mm, pmd);
...
EXPORT_SYMBOL_GPL(__pte_alloc); // profiling
...
EXPORT_SYMBOL_GPL(vm_normal_page); // profiling
...
EXPORT_SYMBOL_GPL(vm_normal_folio); // profiling
...
EXPORT_SYMBOL_GPL(unmap_mapping_folio); // profiling
...
EXPORT_SYMBOL_GPL(do_swap_page); // profiling
...
EXPORT_SYMBOL_GPL(finish_fault); // profiling
 ...
 * return value.  See filemap_fault() and __folio_lock_or_retry().
 */
vm_fault_t (*handle_mm_fault_module)(struct vm_area_struct *vma, unsigned long address, unsigned int flags, struct pt_regs *regs) = NULL;
EXPORT_SYMBOL(handle_mm_fault_module);


KTDEF(my_si_spin_lock);
EXPORT_SYMBOL(my_si_spin_lock_clock);
KTDEF(my_lru_spin_lock);
EXPORT_SYMBOL(my_lru_spin_lock_clock);
vm_fault_t handle_mm_fault(struct vm_area_struct *vma, unsigned long address,
			   unsigned int flags, struct pt_regs *regs)
{
	/*
	if (handle_mm_fault_module && !strcmp(current->comm, "stress"))
		return (*handle_mm_fault_module)(vma, address, flags, regs);
	*/
	if (handle_mm_fault_module)
		return (*handle_mm_fault_module)(vma, address, flags, regs);
...
EXPORT_SYMBOL_GPL(__p4d_alloc); // profiling
...
EXPORT_SYMBOL_GPL(__pud_alloc); // profiling
