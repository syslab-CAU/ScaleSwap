...
EXPORT_SYMBOL_GPL(putback_movable_pages); // profiling
...
EXPORT_SYMBOL_GPL(migration_entry_wait); // profiling
...
EXPORT_SYMBOL_GPL(pmd_migration_entry_wait); // profiling
#endif
...
//		old_lruvec = mem_cgroup_lruvec(memcg, oldzone->zone_pgdat, folio_to_cpu(folio));
		old_lruvec = my_mem_cgroup_lruvec(memcg, oldzone->zone_pgdat, folio_to_cpu(folio));
//		new_lruvec = mem_cgroup_lruvec(memcg, newzone->zone_pgdat, folio_to_cpu(folio));
		new_lruvec = my_mem_cgroup_lruvec(memcg, newzone->zone_pgdat, folio_to_cpu(folio));
		if (old_lruvec->cpu != new_lruvec->cpu) {
//			printk("[%s] hwan - change folio->cpu %d->%d\n", __func__, 
//					old_lruvec->cpu, new_lruvec->cpu);
			folio->flags &= ~(CPUS_MASK << NR_PAGEFLAGS);
			folio->flags |= ((unsigned long)new_lruvec->cpu << NR_PAGEFLAGS);
		}
...
EXPORT_SYMBOL_GPL(migrate_pages); // profiling
...
EXPORT_SYMBOL_GPL(alloc_migration_target); // profiling
...
EXPORT_SYMBOL_GPL(migrate_misplaced_page); // profiling
