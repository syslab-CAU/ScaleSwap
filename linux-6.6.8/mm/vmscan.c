...
EXPORT_SYMBOL_GPL(vm_swappiness); // profiling
...
EXPORT_SYMBOL(free_shrinker_info);
...
EXPORT_SYMBOL(alloc_shrinker_info);
...
EXPORT_SYMBOL_GPL(set_shrinker_bit); // profiling
...
EXPORT_SYMBOL(reparent_shrinker_deferred);
...
EXPORT_SYMBOL_GPL(zone_reclaimable_pages); // profiling
...
EXPORT_SYMBOL_GPL(reclaim_throttle); // profiling
...
EXPORT_SYMBOL_GPL(remove_mapping); // profiling
...
EXPORT_SYMBOL_GPL(folio_putback_lru); // profiling
	...
	while (!list_empty(folio_list)) {
		...
		if (folio_test_anon(folio) && folio_test_swapbacked(folio)) {
			if (!folio_test_swapcache(folio)) {
				...
				printk(KERN_ALERT "[%s] kern, current->comm=%s\n", __func__, current->comm);
...
EXPORT_SYMBOL_GPL(reclaim_clean_pages_from_list); // profiling
...
EXPORT_SYMBOL_GPL(folio_isolate_lru); // profiling
...
EXPORT_SYMBOL(move_folios_to_lru);
...
//	struct lruvec *lruvec = mem_cgroup_lruvec(memcg, pgdat, folio_to_cpu(folio));
	struct lruvec *lruvec = my_mem_cgroup_lruvec(memcg, pgdat, folio_to_cpu(folio));
	DEFINE_MAX_SEQ(lruvec);
...
static void set_initial_priority(struct pglist_data *pgdat, struct scan_control *sc)
{
	...
	struct lruvec *lruvec = mem_cgroup_lruvec(NULL, pgdat, -1);
...
EXPORT_SYMBOL_GPL(try_to_free_pages); // profiling
...
EXPORT_SYMBOL(mem_cgroup_shrink_node);
...
static void kswapd_age_node(struct pglist_data *pgdat, struct scan_control *sc)
{
	...
	lruvec = mem_cgroup_lruvec(NULL, pgdat, -1);
...
void (*shrink_node_module)(pg_data_t *pgdat, struct scan_control *sc);
EXPORT_SYMBOL(shrink_node_module);
			       ...
			       struct scan_control *sc)
{
	 ...
	 */
	if (shrink_node_module) {
		shrink_node_module(pgdat, sc);
	} else {
...
EXPORT_SYMBOL_GPL(wakeup_kswapd); // profiling
...
EXPORT_SYMBOL_GPL(node_reclaim_mode); // profiling
...
EXPORT_SYMBOL_GPL(sysctl_min_unmapped_ratio); // profiling
...
EXPORT_SYMBOL_GPL(sysctl_min_slab_ratio); // profiling
...
EXPORT_SYMBOL_GPL(node_reclaim); // profiling
#endif
