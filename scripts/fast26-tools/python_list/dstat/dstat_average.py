...
parser.add_argument("-p", "--print_dstat", action='store_true', help="print dstat lines.")
parser.add_argument("-d", "--delete_zero", action='store_true', help="계산시 0은 제외.")
parser.add_argument("-z", "--wjftkvudrbs", action='store_true', help="절사 평균.")
...
def get_average_csv(file, human_readable, print_result, delete_zero, wjftkvudrbs):
    start_cal = False
    header_list = []
    line_list = []
    ...
    for line in file.readlines():
        if start_cal == False:
            ...
            header_top_line_num += 1
            if "read" in line and not "output" in line:
                start_cal = True
                header_list = line.replace('"', "").strip().split(",")
            continue
        element_list = list(map(float, line.split(",")))
        cnt += 1
        bandwidth_line.append(element_list)

    if cnt == 0:
        print("error: 측정된 dstat이 없음")
        exit()

    total_list = [[] for _ in range(len(header_list))]
    cnts = [0] * len(header_list)
    for line in bandwidth_line:
        if print_result:
            print(line)

        for i in range(len(line)):
            if not delete_zero or line[i] != 0:
                cnts[i] += 1
                total_list[i].append(line[i])
    ...
    cur_header_top = ""
    ret = {}
    print(len(bandwidth_line) // 60, len(bandwidth_line) % 60)
    for i in range(len(header_list)):
        ...
        if cnts[i] != 0:
            if wjftkvudrbs:
                ...
                total_list[i].sort()
                cur_list = total_list[i][cut_cnt : cnts[i] - cut_cnt]
                if len(cur_list):
                    div_result = round( sum(cur_list) / len(cur_list), 2)
                    print(header_list[i], div_result, sum(total_list[i]), sum(cur_list), len(cur_list))
                else:
                    div_result = round( sum(total_list[i]) / cnts[i], 2)
                    print(header_list[i], div_result)

                # round(total_list[i] / cnts[i], 2)
            else:
                div_result = round(sum(total_list[i][1:]) / (cnts[i] - 1), 2)
                # print(div_result)
        else:
            div_result = 0
        if human_readable:
...
if __name__ == "__main__":
    ...
    if "csv" in args.filename.name:
        ret = get_average_csv(args.filename, args.human_readable, args.print_dstat, args.delete_zero, args.wjftkvudrbs)
    else:
