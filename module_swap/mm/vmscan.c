	...
	while (!list_empty(folio_list)) {
		...
		if (folio_test_anon(folio) && folio_test_swapbacked(folio)) {
			if (!folio_test_swapcache(folio)) {
				...
				// worker가 worker를 호출하는 것을 제외하려면 worker만을 위한 swap 공간을 할당해 줘야 함 (100 page 정도...)
...
//				}
		...
		if (folio_mapped(folio)) {
			...
			if (cpu == si_type) { // 이미 worker 가 수행 중
	...
	while (!list_empty(folio_list)) {
		...
		if (folio_test_anon(folio) && folio_test_swapbacked(folio)) {
			if (!folio_test_swapcache(folio)) {
				...
				bool add_to_swap_ret;
//				if (this_cpu_read(p_nr_swap_pages) < nr_pages + 50) { // worker가 worker를 호출하는 것을 제외하려면 worker만을 위한 swap 공간을 할당해 줘야 함 (100 page 정도...)
//					int patient = 10;
//					while (per_cpu(p_nr_swap_pages, si_type) <= nr_pages + 150) {
//						si_type = get_random_u32_inclusive(get_cpu_to_numa_min[node], get_cpu_to_numa_max[node]); 
//						patient--;
...
//				}
				...
				if (!add_to_swap_ret) { 
					...
					count_vm_event(THP_SWPOUT_FALLBACK);
#endif 
					if (si_type != cpu) { // 이미 worker가 수행 중
						per_cpu(p_nr_swap_pages, si_type) -= folio_nr_pages(folio);
						add_to_swap_ret = add_to_swap_runner(folio, si_type);
					} else {
						add_to_swap_ret = add_to_swap(folio);
...
/*
lru_add_drain 아님
free_unref_page_list 아님
mem_cgroup_uncharge_list 아님
*/
