...
KTDEF(swap_writepage_fs);
KTDEF(swap_writepage_bdev_sync);
KTDEF(swap_writepage_bdev_async);
 ...
 */
KTDEF(__swap_writepage);
int _k_swap_writepage(struct page *page, struct writeback_control *wbc)
{
	struct folio *folio = page_folio(page);
	int ret;
