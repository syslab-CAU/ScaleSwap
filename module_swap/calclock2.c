...
{
        char char_buff[100], char_buff2[100];
        int percentage;
        static ktime_t totaltime = 1;
        u64 total_ns, per_thread_ns;
        u64 minutes, seconds, milliseconds;
        u64 pt_minutes, pt_seconds, pt_milliseconds;
        u64 remainder_ns;
        ...
        /* 전체 시간(ns)을 분, 초, 밀리초로 변환 */
        total_ns = ktime_to_ns(time) / num_online_cpus();
        minutes = total_ns / (60ULL * 1000000000ULL);
        remainder_ns = total_ns % (60ULL * 1000000000ULL);
        seconds = remainder_ns / 1000000000ULL;
        remainder_ns %= 1000000000ULL;
        milliseconds = remainder_ns / 1000000ULL;
        ...
        printk(KERN_CONT "%s is called ", func_name);
        printk(KERN_CONT "%s times, ", separate_num(count, char_buff));
        printk(KERN_CONT "and the time interval is %llu min %llu sec %llu ms (%d.%.2d%%)\n",
...
//        milliseconds = remainder_ns / 1000000ULL;
//
//        /* per thread 시간(ns)도 동일하게 변환 */
//        per_thread_ns = ktime_to_ns(time) / num_online_cpus();
//        pt_minutes = per_thread_ns / (60ULL * 1000000000ULL);
//        remainder_ns = per_thread_ns % (60ULL * 1000000000ULL);
//        pt_seconds = remainder_ns / 1000000000ULL;
//        remainder_ns %= 1000000000ULL;
//        pt_milliseconds = remainder_ns / 1000000ULL;
//
//        printk("%s", "");
//        while (depth--)
//                printk(KERN_CONT "   ");
//        printk(KERN_CONT "%s is called ", func_name);
//        printk(KERN_CONT "%s times, ", separate_num(count, char_buff));
//        printk(KERN_CONT "and the time interval is %llu min %llu sec %llu ms (per thread is %llu min %llu sec %llu ms) (%d.%.2d%%)\n",
//               minutes, seconds, milliseconds,
//               pt_minutes, pt_seconds, pt_milliseconds,
//               percentage / 100, percentage % 100);
//}
